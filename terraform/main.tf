...
# Control node runs Ansible, ansible-lint, and boto3 — heavier package footprint
resource "aws_instance" "control_node" {
  ami                         = data.aws_ami.ubuntu_2204.id
  instance_type               = var.instance_type
  key_name                    = aws_key_pair.cis_lab.key_name
  vpc_security_group_ids      = [aws_security_group.cis_lab.id]
...
# Managed nodes are hardening targets — only python3 is required for Ansible
